...
class App extends Component {
  ...
  componentDidMount() {
    this.start = setInterval(this.tick, 1000)
    console.log('mount')
  }
  ...
  tick = () => {
    this.setState(prevState =>
      prevState.timeSeconds > 0
        ? {
            timeSeconds: prevState.timeSeconds - 1,
          }
        : {timeSeconds: 0, gameOver: true},
    )
  }

  display = id => {
    this.setState({thumbList: id})
  }

  match = id => {
    const {random} = this.state
    const number = Math.ceil(Math.random() * imagesList.length)
    const randomImageList = imagesList[number]
    if (id === random.id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        random: randomImageList,
      }))
    } else {
      clearInterval(this.start)
  ...
  render() {
    const {thumbList, random, score, gameOver, timeSeconds} = this.state
    // console.log('render')
    const ImgUrl = random.imageUrl

    const listOfThumbNail = imagesList.filter(
      each => each.category === thumbList,
    )
      ...
      <>
          ...
          <ul className="cont">
            ...
            <li className="score-time-cont">
              ...
              <img
                ...
                className="timer"
              />
              <p className="seconds">{timeSeconds} sec</p>
            </li>
          </ul>
        </nav>
        ...
        {gameOver === false ? (
          <div className="bgContainer">
            ...
            <ul className="table-tab-cont">
              {tabsList.map(each => (
                <TabList
                  item={each}
                  key={each.tabId}
                  displayContent={this.display}
                  activeTab={thumbList === each.tabId}
                />
              ))}
            </ul>
            <ul className="list-cont">
              {listOfThumbNail.map(each => (
                <ThumbnailList
                  item={each}
                  key={each.id}
                  matchPair={this.match}
                />
              ))}
            </ul>
          </div>
        ) : (
          <div className="bgScoreContainer">
            <div className="score-card-cont">
              ...
              <button className="btn-again" type="button" onClick={this.reGame}>
                <img
